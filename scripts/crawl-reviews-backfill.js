...
async function analyzeWithGemini(reviews) {
  ...
  for (let i = 0; i < reviews.length; i += batchSize) {
    ...
    const prompt = `다음은 화장품(컨실러) 리뷰들입니다. 각 리뷰의 감성을 분석해주세요.
...
async function main() {
  ...
  for (const product of products) {
    ...
    if (product.platform === 'cafe24') {
      ...
      while (true) {
        ...
        try {
          ...
          if (!product.thumbnail_url && reviews[0].product_image_url) {
            ...
            console.log(`[백필] 썸네일 저장: ${thumb.substring(0, 60)}...`);
      ...
      console.log(`\n[백필] 총 ${allReviews.length}건 수집`);
      if (!allReviews.length) { console.log('[백필] 수집 리뷰 없음.'); continue; }

      // Gemini AI 감성분석
      console.log(`\n[백필] Gemini AI 감성분석 시작...`);
      const analyzed = await analyzeWithGemini(allReviews);
      console.log(`[백필] 분석 완료: ${analyzed.length}건`);
    ...
    if (product.platform === 'naver') {
      console.log(`\n========================================`);
      console.log(`[백필] ${product.brand_name} ${product.product_name}`);
      console.log(`========================================`);
      ...
      if (!product.thumbnail_url) {
        ...
        if (thumb) {
          ...
          console.log(`[백필] 네이버 썸네일 저장: ${thumb.substring(0, 60)}...`);
        }
      }
      ...
      if (!naverPayload) {
        console.log('[백필] 네이버 리뷰 API 파라미터 캡처 실패');
        await page.close();
        continue;
      }
      
      console.log(`[백필] 네이버 파라미터 획득: merchant=${naverPayload.checkoutMerchantNo}, origin=${naverPayload.originProductNo}`);
    ...
    if (product.platform !== 'oliveyoung') { console.log(`[백필] ${product.brand_name}: 미지원 플랫폼 (${product.platform})`); continue; }

    const goodsMatch = product.page_url.match(/goodsNo=([A-Za-z0-9]+)/);
    if (!goodsMatch) { console.log('[백필] goodsNo 추출 실패'); continue; }
    const goodsNo = goodsMatch[1];

    console.log(`\n========================================`);
    console.log(`[백필] ${product.brand_name} ${product.product_name} (${goodsNo})`);
    console.log(`========================================`);
    ...
    console.log('[백필] 페이지 방문하여 세션 획득 중...');
    await page.goto(product.page_url, { waitUntil: 'networkidle2', timeout: 60000 });
